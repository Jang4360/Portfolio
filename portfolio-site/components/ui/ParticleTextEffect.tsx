...
const DEFAULT_WORDS = ["측정 가능한 가치를 만드는 개발자"];
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
    ...
    const pixelSteps = 4; // Finer detail for Korean text
    ...
    const nextWord = (word: string, canvas: HTMLCanvasElement) => {
        ...
        offscreenCtx.fillStyle = "#E1E1E1";
        // Reduced font size to fit single line
        offscreenCtx.font = "bold 60px 'Noto Sans KR', sans-serif";
        ...
        for (const coordIndex of coordsIndexes) {
            ...
            if (alpha > 0) {
                ...
                if (particleIndex < particles.length) {
                    ...
                    // Spawn from center instead of random far places for faster convergence
    ...
    const animate = () => {
        ...
        // REMOVED auto-advance word logic:
        // Only renders the first word (which is now the full sentence) and keeps it.
    ...
    useEffect(() => {
        ...
        // Increased canvas width for long text and height for safety
        canvas.width = 1200;
        canvas.height = 300;
